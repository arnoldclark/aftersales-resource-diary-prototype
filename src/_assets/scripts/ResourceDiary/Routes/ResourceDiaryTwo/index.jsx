...
const ResourceDiaryTwo = ({branches, selectedBranch, setBranch, defaultHours, resourceHours, activeOverrides}) => {
  ...
  const highlightResource = (booked, resource) => {
    ...
    if(percentage >= 50 && percentage < 100) {
      return `ch-bg--ac-orange-1 ch-text--bold`;
      // return `ch-bg--ac-orange-1 ch-text--bold`;
    }

    if(percentage >= 100) {
      return `ch-color--grey-5`
    }
    ...
    // return `ch-bg--ac-green ch-color--white ch-text--bold`;
    return `ch-bg--ac-green-1 ch-text--bold`;
    // return `ch-ba--1 ch-bc--grey-4`
  }

  const renderNotification = (booked, resource) => {
    const percentage = (100 * booked) / resource;

    if(percentage >= 50 && percentage < 100) {
      return `br-resource__notification--nearly-booked`;
    }

    if(percentage >= 100 || resource < 1) {
      return `br-resource__notification--booked`
    }

    return `br-resource__notification--available`;
  }
      ...
      <tr key={resource.id}>
        <th className="ch-bg--grey-2 ch-text--left">{resource.name}</th>
          {resource.hours.map((hour, hourIndex) => (
            <td key={hourIndex} className={classNames('ch-text--center br-resource', highlightResource(hour.booked, hour.resource))}>
              <span className="ch-display--block ch-text--center">
                {hour.booked}/{hour.resource}
              </span>
              {/* <div className={classNames('br-resource__notification', renderNotification(hour.booked, hour.resource))}></div> */}
            </td>
          ))}
      </tr>
    ));
...
export default ResourceDiaryTwo;