...
const ContactCentreView = ({branches, selectedBranch, setBranch, defaultHours, resourceHours, activeOverrides}) => {
  ...
  const highlightResource = (booked, resource) => {
    ...
    if(percentage >= 50 && percentage < 100) {
      ...
      // return `ch-bg--ac-green-1 ch-color--white ch-text--bold`;
      // return `ch-bg--ac-orange ch-color--white ch-text--bold`;
    }

    if(percentage >= 100 || resource === 0) {
      return `ch-bg--white ch-color--grey-5 ch-text--bold`
    }
  ...
  const renderNotification = (booked, resource) => {
    ...
    if(percentage >= 50 && percentage < 100) {
      return `br-notification--amber`;
    }

    return `br-notification--green`;
  }
      ...
      <tr key={resource.id}>
        <th className="ch-bg--grey-2 ch-text--left">{resource.name}</th>
          {resource.hours.map((hour, hourIndex) => (
            <td key={hourIndex} className={classNames(`ch-text--center`, highlightResource(hour.booked, hour.resource))}>
              {/* { hour.booked < hour.resource &&
                <span className={classNames(`br-notification ch-mr--1`, renderNotification(hour.booked, hour.resource))}></span>
              } */}
              {hour.booked}/{hour.resource}
            </td>
          ))}
...
export default ContactCentreView;